...
function DashboardMapHarvestSubmodule({
  ...
  // #region data
  // Stable across renders unless polygon actually changes, so it doesn't
  // reset useTimeSeriesChart's onLoadData identity (and re-trigger the
  // chart's load effect) on every unrelated re-render.
  const buildBody = useCallback(() => ({ polygon }), [polygon]);
  const {
  ...
  } = useTimeSeriesChart(endpoint, "frequency", {
    method: "POST",
    body: buildBody,
  });
